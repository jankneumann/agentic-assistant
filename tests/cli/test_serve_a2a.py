"""Tests for `assistant serve --a2a` wiring (P6 a2a-server).

Spec scenarios: cli-interface (CLI serve Subcommand — --a2a flag).
"""
...
def test_serve_a2a_flag_enables_a2a_with_base_url():
    result, calls = _invoke_serve(
        ["-p", "personal", "-r", "coder", "--a2a", "--port", "9001"]
    )
...
def test_serve_without_flag_keeps_legacy_call_shape():
    """No --a2a → make_app is called with NO extra kwargs (legacy shape)."""
    result, calls = _invoke_serve(["-p", "personal", "-r", "coder"])
    assert result.exit_code == 0, result.output
    assert calls[0]["kwargs"] == {}


def test_serve_a2a_announces_agent_card_url():
    result, _ = _invoke_serve(["-p", "personal", "-r", "coder", "--a2a"])
    assert result.exit_code == 0, result.output
    assert "/.well-known/agent-card.json" in result.output


def test_serve_help_documents_a2a_flag():
    serve_cmd = main.commands["serve"]
    param_names = {p.name for p in serve_cmd.params}
    assert "enable_a2a" in param_names

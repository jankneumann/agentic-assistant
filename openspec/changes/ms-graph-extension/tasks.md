...
- [x] 1.1 Write tests for `CloudGraphClient` Protocol shape
  ...
  **Dependencies**: None
- [x] 1.2 Create `src/assistant/core/cloud_client.py` —
  `CloudGraphClient` Protocol with `get`, `post`, `paginate`,
  `get_bytes`, `health_check`
  **Dependencies**: 1.1

- [x] 1.3 Write tests for `MSALStrategy` Protocol +
...
- [x] 1.4 Write tests for `ClientCredentialsStrategy` (mock
...
- [x] 1.5 Write tests for token cache file discipline (tmpfile
...
- [x] 1.6 Write tests for `create_msal_strategy` factory
...
- [x] 1.7 Write tests asserting auth errors do NOT retry
...
- [x] 1.8 Create `src/assistant/core/msal_auth.py` — `MSALStrategy`
...
- [x] 1.9 Write tests for `GraphClient` GET/POST request shape
...
- [x] 1.10 Write tests for `paginate()` (`@odata.nextLink`-chasing,
...
- [x] 1.11 Write tests for resilience integration (per-extension
...
- [x] 1.12 Write tests for 401 token refresh + replay
...
- [x] 1.13 Write tests for `GraphAPIError` sanitization +
  `health_check`
...
- [x] 1.16 Add `msal>=1.28` and `respx>=0.21` to `pyproject.toml`;
  run `uv sync` to lock
  **Dependencies**: 1.8, 1.15

## 2. wp-ms-graph — Real ms_graph extension

- [x] 2.1 Write tests for `ms_graph` extension tool surface (presence,
...
- [x] 2.2 Write tests for `search_people`, `get_my_profile`,
...
- [x] 2.3 Write tests for `ms_graph` HealthStatus derivation
...
- [x] 2.4 Replace `src/assistant/extensions/ms_graph.py` stub with
...
- [x] 3.2 Write tests for `list_messages`, `read_message`,
...
- [x] 3.3 Write tests for `send_email` write tool body shape
  **Spec scenarios**: ms-extensions / "send_email POSTs to
  /me/sendMail with the expected body shape"
  **Dependencies**: 1.14, 1.15

- [x] 3.4 Write tests for outlook HealthStatus derivation
  **Spec scenarios**: ms-extensions / "Real extension reports OK
  when breaker is CLOSED", "Real extension reports UNAVAILABLE when
  breaker is OPEN"
  **Dependencies**: 1.14

- [x] 3.5 Replace `src/assistant/extensions/outlook.py` stub with
  real `OutlookExtension`: six tools, default scopes, dual wrappers,
  health_check from breaker
  **Dependencies**: 3.1, 3.2, 3.3, 3.4

- [x] 3.6 Add `tests/fixtures/graph_responses/outlook/` JSON fixtures
  **Dependencies**: None

## 4. wp-teams — Real teams extension

- [x] 4.1 Write tests for teams tool surface (read + write, dual
  format)
  **Spec scenarios**: ms-extensions / "Tool list includes read and
...
- [x] 4.2 Write tests for `list_chats`, `list_channel_messages`,
...
- [x] 4.3 Write tests for `post_chat_message` body shape
  **Spec scenarios**: ms-extensions / "post_chat_message POSTs to
  /chats/{chatId}/messages"
  **Dependencies**: 1.14, 1.15

- [x] 4.4 Write tests for teams HealthStatus derivation
  **Dependencies**: 1.14

- [x] 4.5 Replace `src/assistant/extensions/teams.py` stub with
  real `TeamsExtension`
  **Dependencies**: 4.1, 4.2, 4.3, 4.4

- [x] 4.6 Add `tests/fixtures/graph_responses/teams/` JSON fixtures
  **Dependencies**: None

## 5. wp-sharepoint — Real sharepoint extension (read-only)

- [x] 5.1 Write tests asserting NO write tools are present
  **Spec scenarios**: ms-extensions / "Tool list contains only read
  tools"
  **Design decisions**: D6
  **Dependencies**: 1.14, 1.15

- [x] 5.2 Write tests for `search_sites`, `list_documents`,
...
- [x] 5.3 Write tests for sharepoint HealthStatus derivation
  **Dependencies**: 1.14

- [x] 5.4 Replace `src/assistant/extensions/sharepoint.py` stub with
  real `SharepointExtension` (read-only)
  **Dependencies**: 5.1, 5.2, 5.3

- [x] 5.5 Add `tests/fixtures/graph_responses/sharepoint/` JSON
  fixtures
  **Dependencies**: None

## 6. wp-msaf-harness — MS Agent Framework harness implementation

- [x] 6.1 Write tests asserting `MSAgentFrameworkHarness.create_agent`
...
- [x] 6.2 Write tests for `create_agent` instructions composition +
  tool union
...
- [x] 6.3 Write tests for `invoke` returning string + exception
  propagation
...
- [x] 6.4 Write tests for `spawn_sub_agent` (sub-role prompt, return
  value)
...
- [x] 6.5 Write tests for capability consumption (ToolPolicy,
...
- [x] 6.6 Write tests for `@traced_harness` decorator integration on
...
- [x] 6.7 Replace `src/assistant/harnesses/sdk/ms_agent_fw.py` stub
...
- [x] 7.2 Locate existing tests asserting
...
- [x] 7.3 Update `personas/_template/.gitignore` to add `.cache/` so
  token cache files never get committed
  **Design decisions**: D2
  **Dependencies**: None

- [x] 7.4 Add a startup check (or test) that asserts
...
- [x] 7.5 Add `tests/test_extensions_dual_format.py` parameterized
...
- [x] 7.6 Add opt-in `tests/integration/test_graph_smoke.py` (gated
  on `RUN_GRAPH_TESTS=1`)
  **Design decisions**: D7
  **Dependencies**: 6.7

- [x] 7.7 Update `openspec/roadmap.md` to flip P5 from `pending` to
...
- [x] 7.8 Run quality gates per "Landing the Plane" (CLAUDE.md):
...
- [x] 7.9 File P5b follow-up issues for deferred scope:
...
- [x] 8.1.1 Write tests for Retry-After honoring on 429 and 503
...
- [x] 8.1.2 Implement Retry-After honoring in GraphClient (parse
  header, sleep up to indicated value before P9 backoff applies)
  **Dependencies**: 8.1.1

- [x] 8.1.3 Write tests for per-request httpx.Timeout default
...
- [x] 8.1.4 Implement timeout configuration in GraphClient
  constructor; pass to httpx.AsyncClient
  **Dependencies**: 8.1.3

- [x] 8.1.5 Write tests for trace_graph_call observability span
...
- [x] 8.1.6 Add `trace_graph_call(...)` method to the P4
  observability provider interface; implement in noop and
  Langfuse providers; wire into GraphClient request loop
  **Dependencies**: 8.1.5

- [x] 8.1.7 Write tests for empty 202/204 body handling (POST returns
...
- [x] 8.1.8 Implement empty-body handling in GraphClient.get/post —
  short-circuit JSON parsing for 202/204/empty-body 200 responses
  **Dependencies**: 8.1.7

- [x] 8.1.9 Write tests confirming GraphAPIError subclasses
...
- [x] 8.1.10 Refactor GraphAPIError to extend httpx.HTTPStatusError
  (preserve typed error_code, request_id, message fields while
  satisfying P9's classifier)
  **Dependencies**: 8.1.9

- [x] 8.1.11 Write tests for retry_safe parameter (False bypasses
...
- [x] 8.1.14 Implement CloudGraphClient.get_bytes (Protocol) and
  GraphClient.get_bytes (impl) — streaming download, max_bytes
  enforcement, tempfile result, partial cleanup on abort
  **Dependencies**: 8.1.13

- [x] 8.1.15 Write tests for paginate raising on page_ceiling
...
- [x] 8.1.16 Refactor paginate to raise GraphAPIError(error_code=
  "page_ceiling_exceeded") on ceiling, after warning log, instead
  of yielding-and-returning
  **Dependencies**: 8.1.15

- [x] 8.1.17 Write tests for asyncio.to_thread wrapping of MSAL sync
...
- [x] 8.1.18 Wrap all synchronous MSAL calls in
  InteractiveDelegatedStrategy and ClientCredentialsStrategy with
  asyncio.to_thread
  **Dependencies**: 8.1.17

- [x] 8.1.19 Write tests for atomic tmp-file mode 0o600 creation
...
- [x] 8.1.20 Refactor token cache write path to use os.open with
  the right mode flags from creation; fail-fast on stale tmp
  **Dependencies**: 8.1.19

- [x] 8.1.21 Write tests for persona-repo .gitignore verification
...
- [x] 8.1.22 Implement gitignore check before any token cache
  write; fail with MSALAuthenticationError when missing
  **Dependencies**: 8.1.21

- [x] 8.1.23 Update msal-auth factory to read auth.ms via
...
- [x] 8.2.1 Write tests for tool input URL-encoding and validation
...
- [x] 8.2.2 Implement URL-encoding helper (urllib.parse.quote with
  safe="") and input validator; use in every tool that interpolates
  IDs into paths
  **Dependencies**: 8.2.1
  Applies to: 2.4, 3.5, 4.5, 5.4

- [x] 8.2.3 Write tests for scope override REPLACE semantics
...
- [x] 8.2.4 Implement scope resolution helper using REPLACE
  semantics; use in all four real extension constructors
  **Dependencies**: 8.2.3
  Applies to: 2.4, 3.5, 4.5, 5.4

- [x] 8.2.5 Write tests for tool invocation with OPEN breaker
...
- [x] 8.2.6 Wrap each extension's tool method with breaker-state
  check; raise structured error when breaker is OPEN
  **Dependencies**: 8.2.5
  Applies to: 2.4, 3.5, 4.5, 5.4

- [x] 8.2.7 (outlook) Update _send_email to pass retry_safe=False
...
- [x] 8.2.8 (teams) Update _post_chat_message to pass
  retry_safe=False when calling client.post
  **Design decisions**: D18
  **Dependencies**: 8.1.12
  Applies to: 4.5

- [x] 8.2.9 (sharepoint) Update _download_document to call
...
- [x] 8.3.1 Write tests for MemoryPolicy snippet injection (snippets
...
- [x] 8.3.2 Implement MemoryPolicy.get_recent_snippets call in
...
- [x] 8.4.1 Write tests for extended factory contract
...
- [x] 8.4.2 Refactor `create_extension` factory contract across
...
- [x] 8.4.3 Update PersonaRegistry.load_extensions to pass
...
- [x] 8.5.1 Verify the dual-format parity test (existing 7.5)
...
- [x] 9.1.1 Write tests for `ObservabilityProvider.trace_graph_call`
...
- [x] 9.1.2 Add `trace_graph_call(...)` to `ObservabilityProvider`
...
- [x] 9.1.3 Write tests for `GraphClient` async context-manager and
...
- [x] 9.1.6 Implement trusted-host validation in `GraphClient`
...
- [x] 9.1.7 Write tests for past/malformed Retry-After header
...
- [x] 9.1.8 Extend Retry-After parsing in `GraphClient` to detect
  past HTTP-dates and malformed values; emit a structured warning
  with sanitized header value
  **Dependencies**: 9.1.7

- [x] 9.1.9 Write test for the measurable msal concurrency
...
- [x] 9.2.1 Write test for real factory called with `persona=None`
...
- [x] 9.2.2 Implement the `persona is None` short-circuit at the
...
- [x] 9.3.1 Write tests asserting list-tool pagination discipline
...
- [x] 9.3.2 Audit each list-tool implementation
...
- [x] 9.3.3 Write tests for per-tool page_ceiling description
...
- [x] 9.3.4 Update each list-tool's StructuredTool description to
...
- [x] 9.4.1 Validate the pinned `agent-framework` version
...
- [x] 9.4.2 Update task 8.1.5/8.1.6 cross-references: 8.1.5 is the
...
The 112 task checkboxes above were ticked en bloc during the cleanup phase (2026-05-09), not progressively during implementation. Background-Agent dispatch in the implement-feature parallel-Agent tier did not include "update tasks.md as you go" in dispatched-agent prompts, so the boxes remained unchecked while the work was being done.

Authoritative coverage is recorded in `change-context.md`, which traces all 51 SHALL/MUST clauses across 7 spec deltas to source files and test files with zero gaps. The full convergence loop ran four IMPL_REVIEW rounds (16 + 7 + 3 + 0 findings) plus VALIDATE (1 finding) for a total of 17 verified real bugs fixed across 22 candidates raised. Final pytest suite is green at 763 passed, 3 skipped. CI on PR #24 passed at commit b05bad4. Security review parsed zero findings.

Follow-up worth filing against `agentic-coding-tools` (the canonical skill source): teach the implement-feature parallel-Agent tier to either update tasks.md after each Agent completes, or drop tasks.md as a status artifact in favor of change-context.md.

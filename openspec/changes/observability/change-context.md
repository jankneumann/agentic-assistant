...
| observability.3 | specs/observability/spec.md:74-92 | `HarnessAdapter.invoke()` MUST emit exactly one `trace_llm_call` per invocation (after await) with model/persona/role/tokens/duration; on exception still emit with `metadata={"error": <type>}` then re-raise. | --- | D3 | src/assistant/telemetry/decorators.py, src/assistant/harnesses/sdk/deep_agents.py, src/assistant/harnesses/sdk/ms_agent_fw.py | 2.1, 2.3 | --- |
| observability.4 | specs/observability/spec.md:94-107 | `DelegationSpawner.delegate()` MUST emit `trace_delegation` with parent_role, sub_role, task (sha256 hash if >256 chars), persona, duration_ms, outcome. | --- | D3 | src/assistant/telemetry/decorators.py, src/assistant/delegation/spawner.py | 2.4, 2.6 | --- |
...
| observability.6 | specs/observability/spec.md:130-159 | Every `MemoryManager` public method MUST emit `trace_memory_op` with the fixed op enum; graphiti calls are NOT separately instrumented (single span at MemoryManager boundary). | --- | --- | src/assistant/telemetry/decorators.py, src/assistant/core/memory.py | 3.1, 3.2, 3.3 | --- |

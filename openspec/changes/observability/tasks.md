...
- [ ] **1.7** Write tests for `sanitize(value)` — each ordered regex, including the ordering invariant (lf-key before generic sk), persona-name passthrough, and submodule URL redaction.
  **Spec scenarios**: observability — "Langfuse secret key is redacted before Langfuse public key", "Private submodule URL is redacted", "Persona name is preserved"
...
- [ ] **1.9** Write tests for the assistant context `ContextVar` — set/get, nested scope, async boundary propagation.
...
- [ ] **1.14** Write tests for `factory.get_observability_provider()` — singleton caching across calls, atexit registration, 3-level degradation, one-warning-per-process behavior.
  ...
  **Design decisions**: D1 (singleton lifecycle), D2 (3-level state machine), D6 (atexit registration)
...
- [ ] **2.3** Apply `@traced_harness` to `HarnessAdapter.invoke` abstract base in `src/assistant/harnesses/base.py` and to `DeepAgentsHarness.invoke` in `src/assistant/harnesses/deep_agents.py`.
  **Spec scenarios**: harness-adapter — "Deep Agents harness invocation is traced", "Harness exception still emits trace before propagating"
  **Dependencies**: 2.2
...
- [ ] **3.1** Write tests for memory-op tracing — `MemoryStore` read/write/recall each emits `trace_memory_op` with correct op value and target.
  **Spec scenarios**: observability — "Memory read emits trace_memory_op"
  **Dependencies**: 1.15

- [ ] **3.2** Instrument `src/assistant/core/memory.py` — apply per-method `trace_memory_op` calls for read/write/recall.
  **Dependencies**: 3.1

- [ ] **3.3** Write tests for graphiti-op tracing — `GraphitiClient.add_episode` and query methods emit `trace_memory_op`.
...
- [ ] **3.7** Apply `wrap_structured_tool` inside `src/assistant/extensions/base.py` (or the appropriate base class) so every `as_langchain_tools()` result is wrapped with `tool_kind="extension"`.
  **Spec scenarios**: extension-registry — "Extension tool invocation emits trace_tool_call", "Tool metadata passthrough is preserved"
  **Dependencies**: 3.6
...
- [ ] **3.9** Apply `wrap_structured_tool` inside `src/assistant/http_tools/builder.py` `_build_structured_tool` (or successor) so every HTTP tool is wrapped with `tool_kind="http"`.
  **Dependencies**: 3.8
...
- [ ] **4.3** Write `docs/observability.md` — quickstart (docker-compose up, env vars, run assistant), privacy/sanitization notes, dev-only credential warning, Claude Code Stop hook wiring instructions (pointer to agent-coordinator/scripts/langfuse_hook.py).
  **Design decisions**: D9, D10 (documentation-only Stop hook)
  **Dependencies**: 4.2
...
- [ ] **5.2** Write integration test `tests/telemetry/test_privacy_compliance.py` — exercise telemetry module under the privacy-guard plugin fixtures; assert no filesystem I/O is attempted by any provider path.
  **Design decisions**: Privacy Boundary Compliance
  **Dependencies**: 1.15
...
- [ ] **5.5** Add a CI smoke test (in `.github/workflows/ci.yml` or the equivalent) that spins up Langfuse via `docker-compose.langfuse.yml`, runs a scripted assistant interaction with `LANGFUSE_ENABLED=true`, and asserts spans land in Langfuse via the admin API.

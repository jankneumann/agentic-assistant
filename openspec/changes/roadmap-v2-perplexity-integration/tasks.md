...
- [x] 1.1 Write test: verify `docs/perplexity-feedback.md` exists and
  ...
  **Design decisions**: D6
  **Dependencies**: None
- [x] 1.2 Create `docs/perplexity-feedback.md` by copying the review
  verbatim (from the planning conversation) into the file. Preserve all
  § numbering exactly so `roadmap.md` citations resolve.
  **Dependencies**: 1.1
  **Note**: file added on `openspec/roadmap-v2-reconciliation` branch
  (post-merge) after Codex P1 review caught the omission on PR 3.

## Phase 2 — Roadmap rewrite

- [x] 2.1 Write test: verify `openspec/roadmap.md` phase-sequence table
  ...
  **Dependencies**: 1.2
- [x] 2.2 Write test: verify the Dependency graph section's phase names
  ...
  **Design decisions**: D2
  **Dependencies**: 2.1
- [x] 2.3 Rewrite `openspec/roadmap.md` per design.md:
...
- [x] 3.1 Write test: `openspec validate roadmap-v2-perplexity-integration
  --strict` exits 0.
...
- [x] 4.1 Run `openspec validate roadmap-v2-perplexity-integration
  --strict` and confirm exit 0. Fix any delta-header or scenario-format
  errors.
  **Dependencies**: 3.2, 2.3, 1.2
- [x] 4.2 Run a manual walkthrough: pick three phases from the new
  ...
  **Dependencies**: 4.1
- [x] 4.3 Update the "Current Status" / "What's Not Yet Wired" note at
...
- [x] 5.1 Append Plan-phase session log entry per `/plan-feature` Step
  11.5 (already handled by the skill; listed here for completeness).
  **Dependencies**: 4.3
- [x] 5.2 After archival of this change, the first downstream phase

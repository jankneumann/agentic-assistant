...
| X1 | `add-teacher-role` | non-phase (feature) | in-progress | — | user-requested (2026-04-16) | Add `teacher` role with Feynman + Socratic skill files; `--method` CLI flag and `/method` / `/methods` REPL commands; declare `content_analyzer:*` preferred tools that bind via the archived P3 http-tools layer once ACA aligns its operationIds (`operation_id="search"` on `search_knowledge_base`, `operation_id="knowledge_graph"` on `query_knowledge_graph` — tracked as R5 in `openspec/changes/add-teacher-role/design.md`). Independent of the P-phase DAG — does not block or depend on any phase. |

...
| P3 | `http-tools-layer` | phase | in-progress | §8.2 | perplexity §8.2 + old P2 | `src/assistant/http_tools/` — `/openapi.json`-based discovery with `$ref` resolution (D10), `_build_tool()` Pydantic-model + async-callable generator, auth header handling (D11 structured + legacy compat), registry, `--list-tools` CLI flag, integration tests against mock server under D9 security posture (streaming 10 MiB cap, no redirects, credential redaction) |

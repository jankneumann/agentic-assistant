...
| P26 | `knowledge-clean-room` | phase | in-progress | — | new (ecosystem pillar 3); re-scopes the deferred "cross-persona bridge" | Declassification gateway for clean-room knowledge sharing: policy-driven, audited flow `source persona memory → sanitization (reuse telemetry/sanitize.py) → shared knowledge space → consuming persona/external agent`, with per-fact provenance and revocation. Runtime analogue of the test-time privacy boundary |
